...
#[cfg(test)]
mod test {
    use super::{OrderedMergeJoin,ColumnScan};
    use super::super::{VectorColumn,GenericColumnScan}; // < TODO: is this a nice way to write this use?

    #[test]
    fn test_u64_simple_join() {
        let data1: Vec<u64> = vec![1,3,5,7,9];
        let gcs1 : GenericColumnScan<u64> = GenericColumnScan::new(Box::new(VectorColumn::new(data1)));
        let data2: Vec<u64> = vec![1,5,6,7];
        let gcs2 : GenericColumnScan<u64> = GenericColumnScan::new(Box::new(VectorColumn::new(data2)));
        let data3: Vec<u64> = vec![1,2,3,4,5,6,7,8];
        let gcs3 : GenericColumnScan<u64> = GenericColumnScan::new(Box::new(VectorColumn::new(data3)));

        let mut omj : OrderedMergeJoin<u64> = OrderedMergeJoin::new(vec![Box::new(gcs1),Box::new(gcs2),Box::new(gcs3)]);
