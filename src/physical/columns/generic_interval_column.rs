...
impl<T> GenericIntervalColumn<T> {
    ...
    pub fn new(data: Box<dyn Column<T>>, int_starts: Box<dyn Column<usize>>) -> GenericIntervalColumn<T> {
...
#[cfg(test)]
mod test {
    use super::{Column, IntervalColumn, GenericIntervalColumn};
    ...
    fn test_u64_column() {
        ...
        let gic: GenericIntervalColumn<u64>  = GenericIntervalColumn::new(Box::new(v_data),Box::new(v_int_starts));
        assert_eq!(gic.len(), 9);
