use lookahead_scan::LookaheadScan;

use super::{TableSchema, TrieScan, TrieScanEnum};
use crate::physical::columns::{
    lookahead_scan, RangedColumnScanCell, RangedColumnScanEnum, RangedColumnScanT,
};
...
pub struct TrieFull<'a> {
    ...
    current_col_scan: Option<UnsafeCell<RangedColumnScanT<'a>>>,
        ...
        if self.current_layer.is_none() {
            ...
            macro_rules! init_scans_for_datatype {
                ($variant:ident, $type:ty) => {
                    unsafe {
                        let lookahead_scan = LookaheadScan::<$type>::new(
                            self.trie_scan.as_ref().get().as_mut().unwrap(),
                            current_layer,
                            current_type,
                        );

                        self.current_col_scan = Some(UnsafeCell::new(RangedColumnScanT::$variant(
                            RangedColumnScanCell::new(RangedColumnScanEnum::LookaheadScan(
                                lookahead_scan,
                            )),
                        )));
                    }
...
impl<'a> TrieScan<'a> for TrieFull<'a> {
    ...
    fn get_scan(&self, _index: usize) -> Option<&UnsafeCell<RangedColumnScanT<'a>>> {
...
mod test {
    use crate::physical::{
        ...
        tables::{
            IntervalTrieScan, Trie, TrieFull, TrieJoin, TrieScan, TrieScanEnum, TrieSchema,
            TrieSchemaEntry,
        },
        util::make_gict,
    };
    ...
    fn full_next(scan: &mut TrieFull) -> Option<u64> {
        unsafe {
            if let RangedColumnScanT::U64(rcs) = &(*scan.current_scan()?.get()) {
                rcs.next()
            } else {
    ...
    fn test_u64() {
        ...
        let join_iter = TrieScanEnum::TrieJoin(TrieJoin::new(
            vec![
