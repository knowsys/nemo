...
use super::{
    bytes_buffer::GlobalBytesBuffer,
...
impl<B: GlobalBytesBuffer> GenericRankedPairDictionary<B> {
    ...
    pub(crate) fn add_pair(&mut self, frequent: &[u8], rare: &[u8]) -> AddResult {
        let fid =
        match self.frequent_dict.add_bytes(frequent) {
        ...
        self.pair_dict.add_bytes(Self::pair_bytes(fid, rare).as_slice())
    }
    ...
    pub(crate) fn id_to_pair(&self, id: usize) -> Option<(Vec<u8>, Vec<u8>)> {
        if let Some(pair_bytes) = self.pair_dict.id_to_bytes(id) {
            ...
            match pair_bytes[0] & FTYPE_BIT_MASK {
                0b0000_0000 => { // 1 byte for frequent id
    ...
    pub(crate) fn mark_pair(&mut self, frequent: &[u8], rare: &[u8]) -> AddResult {
        ...
        self.pair_dict.mark_bytes(Self::pair_bytes(fid, rare).as_slice())
    }
...
mod test {
    ...
    fn mark_pair() {
        ...
        assert_eq!(dict.mark_pair(&[3, 4],&[5, 6, 7]), AddResult::Fresh(KNOWN_ID_MARK));
        assert_eq!(dict.mark_pair(&[5, 6],&[5, 6, 7]), AddResult::Fresh(KNOWN_ID_MARK));
        assert_eq!(dict.add_pair(&[3, 4],&[5, 6, 7]), AddResult::Known(KNOWN_ID_MARK));
