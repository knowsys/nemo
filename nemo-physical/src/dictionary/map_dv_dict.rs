...
use super::{
    meta_dv_dict::MetaDvDictionary, tuple_dv_dict::TupleBytesPairDictionary, AddResult, DvDict,
};
...
impl DvDict for MapDvDict {
    ...
    fn datavalue_to_id(&self, _dv: &AnyDataValue) -> Option<usize> {
        unimplemented!("map dictionaries do not support local `datavalue_to_id`; use the callback `datavalue_to_id_with_parent_fn`");
    }
    ...
    fn id_to_datavalue(&self, _id: usize) -> Option<AnyDataValue> {
        unimplemented!("map dictionaries do not support local `id_to_datavalue`; use the callback `id_to_datavalue_with_parent_fn`");
    }
...
fn add_map_datavalue_with_parent(
    ...
    if let Some(map_dict) = myself.as_any_mut().downcast_mut::<MapDvDict>() {
        ...
        panic!("map dictionary called with a subdictionary id that does not belong to a tuple dict; this is a bug")
    }
}
...
fn id_to_map_datavalue_with_parent(
    ...
    Some(MapDataValue::from_chunks(label, values.array_chunks()).into())
}
