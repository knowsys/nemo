...
/// Extract the lexical string content from a plain or language-tagged string value.
///
/// Returns `None` if the value is not a string type.
fn string_content(value: AnyDataValue) -> Option<String> {
    match value.value_domain() {
        ValueDomain::PlainString => Some(value.to_plain_string_unchecked()),
        ValueDomain::LanguageTaggedString => {
            let (s, _) = value.to_language_tagged_string_unchecked();
            Some(s)
        }
        _ => None,
    }
...
///
/// Returns `None` if the argument is not a (language tagged) string.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub struct StringMd5;
impl UnaryFunction for StringMd5 {
    fn evaluate(&self, parameter: AnyDataValue) -> Option<AnyDataValue> {
        let s = string_content(parameter)?;
