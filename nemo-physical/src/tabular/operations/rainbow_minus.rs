...
#[derive(Debug)]
pub struct TrieScanSubtractRainbow<'a> {
    /// [`PartialTrieScan`] from which elements are being subtracted
    trie_main: Box<TrieScanRainbowEnum<'a>>,
    /// Elements that are subtracted
    tries_subtract: Vec<TrieScanRainbowEnum<'a>>,
...
/// Struct containing additional information for the setting up a [`TrieScanSubtractRainbow`] iterator.
...
impl<'a> TrieScanSubtractRainbow<'a> {
    /// Construct new [`TrieScanSubtractRainbow`] object.
    pub fn new(
        trie_main: TrieScanRainbowEnum<'a>,
        tries_subtract: Vec<TrieScanRainbowEnum<'a>>,
        infos: Vec<SubtractInfo>,
    ) -> Self {
        ...
        debug_assert!(tries_subtract
            .iter()
            .all(|trie_subtract| trie_main.arity() >= trie_subtract.arity()));
        debug_assert!(infos
            .iter()
            .all(|info| trie_main.arity() >= info.used_layers.len()));
        debug_assert!(tries_subtract
            .iter()
            .zip(infos.iter())
            .all(|(trie_subtract, info)| info.used_layers.len() == trie_subtract.arity()));
        debug_assert!(infos.iter().all(|info| info.used_layers.is_sorted()));
        debug_assert!(infos
            .iter()
            .all(|info| info.used_layers.iter().all(|&l| l < trie_main.arity())));

        let arity_main = trie_main.arity();
        ...
        let mut column_scans: Vec<UnsafeCell<ColumnScanRainbow<'a>>> =
            Vec::with_capacity(arity_main);

        for layer in 0..arity_main {
            let scan_value_keys = &unsafe { &*trie_main.scan(layer).unwrap().get() }.scan_keys;
            let scan_value_integers =
                &unsafe { &*trie_main.scan(layer).unwrap().get() }.scan_integers;
            let scan_value_doubles =
                &unsafe { &*trie_main.scan(layer).unwrap().get() }.scan_doubles;

            let mut scans_follower_keys = Vec::<Option<&ColumnScanCell<'a, u64>>>::new();
            let mut scans_follower_integers = Vec::<Option<&ColumnScanCell<'a, i64>>>::new();
            let mut scans_follower_doubles = Vec::<Option<&ColumnScanCell<'a, Double>>>::new();
                ...
                tries_subtract.iter().zip(infos.iter()).enumerate()
            {
                ...
                if let Some(used_layer) = used_layer {
                    let scan_subtract_keys =
                        &unsafe { &*trie_subtract.scan(used_layer).unwrap().get() }.scan_keys;
                    let scan_subtract_integers =
                        &unsafe { &*trie_subtract.scan(used_layer).unwrap().get() }.scan_integers;
                    let scan_subtract_doubles =
                        &unsafe { &*trie_subtract.scan(used_layer).unwrap().get() }.scan_doubles;

                    scans_follower_keys.push(Some(scan_subtract_keys));
                    scans_follower_integers.push(Some(scan_subtract_integers));
                    scans_follower_doubles.push(Some(scan_subtract_doubles));
                    ...
                    scans_follower_keys.push(None);
                    scans_follower_integers.push(None);
                    scans_follower_doubles.push(None);
                }
            }

            let new_subtract_scan_keys =
                ColumnScanEnum::ColumnScanSubtract(ColumnScanSubtract::new(
                    scan_value_keys,
                    scans_follower_keys,
                    follow_indices.clone(),
                    subtract_indices.clone(),
                ));
            let new_subtract_scan_integers =
                ColumnScanEnum::ColumnScanSubtract(ColumnScanSubtract::new(
                    scan_value_integers,
                    scans_follower_integers,
                    follow_indices.clone(),
                    subtract_indices.clone(),
                ));
            let new_subtract_scan_doubles =
                ColumnScanEnum::ColumnScanSubtract(ColumnScanSubtract::new(
                    scan_value_doubles,
                    scans_follower_doubles,
                    follow_indices,
                    subtract_indices,
                ));

            let new_scan = ColumnScanRainbow::new(
                new_subtract_scan_keys,
                new_subtract_scan_integers,
                new_subtract_scan_doubles,
            );
...
impl<'a> PartialTrieScanRainbow<'a> for TrieScanSubtractRainbow<'a> {
    fn up(&mut self) {
    ...
    fn down(&mut self, storage_type: StorageTypeName) {
        ...
        for (subtract_index, trie_subtract) in self.tries_subtract.iter_mut().enumerate() {
                ...
                .get(next_layer_subtract)
            {
                if next_layer == next_used_layer {
                    ...
                    trie_subtract.down(storage_type);
                }
            }
        }
        ...
        if next_layer > 0 {
            ...
            self.column_scans[next_layer]
                .get_mut()
                .subtract_enable(storage_type, &equal_values);
        }

        self.trie_main.down(storage_type);
...
#[derive(Debug)]
pub struct TrieScanMinusRainbow<'a> {
    /// [`PartialTrieScan`] from which elements are being subtracted
...
impl<'a> TrieScanMinusRainbow<'a> {
    /// Construct new [`TrieScanMinusRainbow`] object.
    pub fn new(
        trie_left: TrieScanEnum<'a>,
        trie_right: TrieScanEnum<'a>,
    ) -> TrieScanMinusRainbow<'a> {
...
impl<'a> PartialTrieScan<'a> for TrieScanMinusRainbow<'a> {
    fn up(&mut self) {
...
// #[cfg(test)]
// mod test {
//     use super::TrieScanMinusRainbow;
//     use crate::columnar::traits::columnscan::ColumnScanT;
//     use crate::tabular::operations::triescan_minus::{SubtractInfo, TrieScanSubtractRainbow};
...
//     fn diff_next(diff_scan: &mut TrieScanMinusRainbow) -> Option<u64> {
...
//     fn diff_current(diff_scan: &mut TrieScanMinusRainbow) -> Option<u64> {
//         if let ColumnScanT::U64(rcs) = diff_scan.current_scan()? {
//             rcs.current()
//         } else {
//             panic!("type should be u64");
...
//     fn sub_next(sub_scan: &mut TrieScanSubtractRainbow) -> Option<u64> {
...
//     fn sub_current(sub_scan: &mut TrieScanSubtractRainbow) -> Option<u64> {
//         if let ColumnScanT::U64(rcs) = sub_scan.current_scan()? {
//             rcs.current()
//         } else {
//             panic!("type should be u64");
...
//         let mut diff_iter = TrieScanMinusRainbow::new(trie_left_iter, trie_right_iter);
...
//         let mut sub_scan = TrieScanSubtractRainbow::new(
//             trie_scan_main,
//             vec![
//                 trie_scan_sub_1,
//                 trie_scan_sub_2,
