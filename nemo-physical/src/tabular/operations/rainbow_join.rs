...
impl<'a> TrieScanJoinRainbow<'a> {
    ...
    pub fn new(trie_scans: Vec<TrieScanRainbowEnum<'a>>, bindings: &JoinBindings) -> Self {
        ...
        let mut column_scans: Vec<UnsafeCell<ColumnScanRainbow<'a>>> =
            Vec::with_capacity(bindings.num_output_columns());
        ...
        for output_index in 0..bindings.num_output_columns() {
            ...
            // TODO: Consider implementing the optimization with ColumnScanPass
            // TODO: Even better think about whether ColumnScanPass is needed at all?
            let new_union_scan_keys =
                ColumnScanEnum::ColumnScanJoin(ColumnScanJoin::new(column_scans_keys));
            let new_union_scan_integers =
                ColumnScanEnum::ColumnScanJoin(ColumnScanJoin::new(column_scans_integers));
            let new_union_scan_doubles =
                ColumnScanEnum::ColumnScanJoin(ColumnScanJoin::new(column_scans_doubles));
...
impl<'a> PartialTrieScanRainbow<'a> for TrieScanJoinRainbow<'a> {
    fn up(&mut self) {
    ...
    fn down(&mut self, storage_type: StorageTypeName) {
        ...
        debug_assert!(current_layer < self.arity());
        ...
        self.column_scans[current_layer]
            .get_mut()
            .reset(storage_type);
    }
