use crate::{
    ...
    tabular::traits::{partial_trie_scan::PartialTrieScan, table::Table},
...
impl<'a> PartialTrieScan<'a> for TrieScanProject<'a> {
    fn up(&mut self) {
...
mod test {
    ...
    use crate::tabular::traits::{partial_trie_scan::TrieScanEnum, table::Table};
    ...
    fn single_small_hole() {
        ...
        let trie_no_first = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![1, 2], 3)),
        )))
        .unwrap();
        let trie_no_middle = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![0, 2], 3)),
        )))
        .unwrap();
        let trie_no_last = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![0, 1], 3)),
        )))
        .unwrap();
    ...
    fn multiple_big_holes() {
        ...
        let trie_projected = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![0, 3, 5], 6)),
        )))
        .unwrap();
    ...
    fn reorder() {
        ...
        let trie_reordered = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![2, 0, 1], 3)),
        )))
        .unwrap();
    ...
    fn reorder_and_project() {
        ...
        let trie_reordered = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![2, 0], 3)),
        )))
        .unwrap();
    ...
    fn test_duplicates() {
        ...
        let trie_projected = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![0, 2, 4, 1], 5)),
        )))
        .unwrap();
    ...
    fn complex_reorder() {
        ...
        let trie_projected = materialize(&mut TrieScanPrune::new(TrieScanEnum::TrieScanProject(
            TrieScanProject::new(&trie, ProjectReordering::from_vector(vec![2, 0, 3], 4)),
        )))
        .unwrap();
    ...
    fn spurious_tuples_in_reorder_bug() {
        ...
        let a = dict.add_string("139".to_owned()).value().try_into().unwrap();
        let b = dict.add_string("141".to_owned()).value().try_into().unwrap();
        let u = dict.add_string("140".to_owned()).value().try_into().unwrap();
        let v = dict.add_string("134".to_owned()).value().try_into().unwrap();
        ...
        let reordered_trie = materialize(&mut TrieScanPrune::new(project)).unwrap();
    ...
    fn spurious_tuples_in_reorder_mk2_bug() {
        let mut dict = Dict::default();
        let mut intern =
            |term: &str| StorageValueT::U64(dict.add_string(term.to_owned()).value().try_into().unwrap());
    ...
    fn spurious_tuples_in_reorder_bug2() {
        ...
        let rg = dict.add_string("RoleGroup".to_owned()).value().try_into().unwrap();
        let a = dict.add_string("4_1_6".to_owned()).value().try_into().unwrap();
        let b = dict.add_string("4_1_21".to_owned()).value().try_into().unwrap();
        let c = dict.add_string("4_1_22".to_owned()).value().try_into().unwrap();

        let u = dict.add_string("32_1_58".to_owned()).value().try_into().unwrap();
        let v = dict.add_string("32_1_72".to_owned()).value().try_into().unwrap();
        let w = dict.add_string("32_1_81".to_owned()).value().try_into().unwrap();
        let x = dict.add_string("32_1_74".to_owned()).value().try_into().unwrap();
        let y = dict.add_string("32_1_83".to_owned()).value().try_into().unwrap();
        let z = dict.add_string("32_1_60".to_owned()).value().try_into().unwrap();
