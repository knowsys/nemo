...
use crate::{
    ...
    model::{PrimitiveType, PrimitiveValue},
};
        ...
        macro_rules! into_parser {
            ($it:ident, $lcbp:ident) => {{
                let boxed: Box<dyn ColumnBuilderProxy<String>> = match $it {
                    TypeConstraint::Exact(PrimitiveType::Any) | TypeConstraint::AtLeast(PrimitiveType::Any) => Box::new($lcbp.into_parser::<PrimitiveValue>()),
