...
fn f_preds_for_ex_vars(ex_vars_of_rule: &HashSet<&Variable>, rule_name: &String) -> Vec<Tag> {
    ex_vars_of_rule
        .iter()
        .enumerate()
...
fn modified_msa_rule(
    ...
    let pos_vars_of_rule: HashSet<&Variable> = rule.positive_variables();
    ...
    preds_zip_terms.into_iter().for_each(|(f_pred, ex_term)| {
        ...
        pos_vars_of_rule.iter().for_each(|var| {
            let var_as_term: Term = Term::from((*var).clone());
            let s_atom: Atom = Atom::new(s_pred.clone(), Vec::from([ex_term.clone(), var_as_term]));
    ...
    ex_vars_of_rule.iter().enumerate().for_each(|(i, var)| {
        let cons_name: String = format!("_msa_c_{}_{}", rule.name().unwrap(), i);
...
fn f_msa_rules(f_preds: &Vec<Tag>, d_pred: &Tag, x1_term: &Term, x2_term: &Term) -> Vec<Rule> {
    let c_pred: Tag = Tag::from("_msa_C");
    let c_atom: Atom = Atom::new(c_pred, Vec::default());
    ...
    fn apply(self, program: &ProgramHandle) -> Result<ProgramHandle, ValidationReport> {
        ...
        for statement in program.statements() {
            if let Statement::Rule(rule) = statement {
                ...
                let rule_name: String = rule.name().unwrap();
                let f_preds: Vec<Tag> = f_preds_for_ex_vars(&ex_vars_of_rule, &rule_name);
                let modified_msa_rule: Rule =
                    modified_msa_rule(rule, &s_pred, &f_preds, ex_vars_of_rule);
