...
use crate::static_checks::rule_set::{ExistentialRuleAndVariables, RuleAndVariable, RuleSet};
...
impl<'a> AttackedPositionsBuilderInferenceExtended<'a> for RuleSet {
    fn attacked_positions_by_rule_and_var(
        ...
        let mut att_pos: Positions = self.initial_attacked_positions(rule_and_var);
    ...
    fn match_attacking_variables(&self, att_type: AttackingType) -> HashSet<RuleAndVariable> {
        match att_type {
            ...
            AttackingType::Existential => self.existential_rule_and_variables(),
        }
    }
}
