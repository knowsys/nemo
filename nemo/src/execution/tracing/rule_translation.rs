...
///
/// For a normalized rule whose origin cannot be mapped to a rule of the original
/// program (for example because it was generated by a transformation), the index
/// translation falls back to the identity, while [RuleIdTranslation::original_rule]
/// still resolves the rule content through the chain of transformations.
///
...
mod test {
    ...
    #[test]
    fn first_revision_is_original() {
        let original = program_with_rule(Rule::parse("P(?x, ?y) :- Q(?y, ?x) .").unwrap());

        // A revision later derived from the same pipeline recovers the first
        // (original) revision.
        let derived = original.fork().submit().unwrap();
        let recovered = derived.original_revision();
